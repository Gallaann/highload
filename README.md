...
- [Проектирование высоконагруженного сервиса для хостинга проектов и их совместной разработки GitHub](#проектирование-высоконагруженного-сервиса-для-хостинга-проектов-и-их-совместной-разработки-GitHub)
    ...
    - [2. Расчет нагрузки](#2-расчет-нагрузки)
        - [2.1 Продуктовые метрики](#21-продуктовые-метрики)
          - [2.1.1 Целевая аудитория](#211-целевая-аудитория)
          - [2.1.2 Статистика по действиям](#212-статистика-по-действиям)
          - [2.1.3 Среднее количество действий пользователя по типам за период](#213-среднее-количество-действий-пользователя-по-типам-за-период)
        - [2.2 Технические метрики](#22-технические-метрики)
          - [2.2.1 Размер хранения в разбивке по типам данных](#221-размер-хранения-в-разбивке-по-типам-данных)
    ...
    - [5. Логическая схема БД](#5-логическая-схема-бд)
        - [5.1 Схема БД](#51-схема-бд)
        - [5.2 Примерный размер данных](#52-примерный-размер-данных)
...
### 5.2 Примерный размер данных:

Users:

```
    8 (user_id) + 64(email) + 64(username) + 64(password) + 128(avatar_url) = 328 байта
    328 * 100M(кол-во пользователей) = 456 Гб
```

Repositories:

```
    2 * 8(bigint) + 128(files_url) = 144 байт
    144 * 420М = 60.48 Гб
```
...
```
    3 * 8(id) + 63(enum) + 2 * 8(timestamp) = 103 байт
...
```
    3 * 8(bigint) + 8(timestamp) + 128(text) = 160 байт
    160 * 10500М(420М репозиториев, в среднем 5 pull-requests в репозитории и 5 комментариев к pull-requests) = 1.68 Тб
```

Issue:

```
    3 * 8(bigint) + 63(enum) + 64(title) + 512(text) = 663 байт
    663 * 2100M(420М репозиториев, в среднем 5 issue в репозитории) = 1.266 Тб
```
...
```
    3 * 8(bigint) + 8(timestamp) +  128(text) = 160 байт
    160 * 4200M(420М репозиториев, в среднем 5 issue в репозитории и 2 комметария к Issues) = 672 Гб
```
